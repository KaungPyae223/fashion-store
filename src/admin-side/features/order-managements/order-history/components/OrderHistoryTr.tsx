...
const OrderHistoryTr = ({ order }) => {
  ...
  return (
    <tr className="duration-100 hover:bg-gray-800 hover:text-white text-gray-800 bg-white border-y-[12px] border-y-gray-100">
      <td className="p-2 py-4  text-end">{order.id}</td>
      ...
      <td className="text-end p-2 ">{order.total_price} Ks</td>
      <td className="text-end p-2 ">{formatDate(order.order_at)}</td>
      <td className="text-start p-2 px-4 text-nowrap ">{order.packager}</td>
      <td className="text-start p-2 ">{order.delivery}</td>
      <td className="text-center p-2 ">
        <Link href={"/admin/order-history/details/" + order.id}>
          <svg
